...
let
  ...
  pyo = pkgs.python39Packages.buildPythonPackage rec {
    ...
    doCheck = false;
    # FIXME(create custom setup.py to ensure pyo is compiled with jack support.
    #       this should actually be done with flags parsed to "python setup.py install",
    #       but I don't know yet how to do this in nix.)
    preBuild = ''
        rm setup.py;
        cat >setup.py <<'EOF'
...
in
  ...
  buildPythonPackage rec {
    ...
    src = fetchFromGitHub {
      owner = "audiowalkman";
      repo = "walkman";
      rev = "ee04d7f921b34806e7e4430067f6a8c1d0bff1ff";
      sha256 = "sha256-AAGq7d2Mqi7n6Q14Q98Y8PqkZhfvJTZVFp+rwNxTMwo=";
    };
